...
jQuery(document).ready(function($){
    ...
    $("a.mvb_row_settings").live('click', function(e){
        ...
        var data = {
                    ...
                    padding_top: therow.attr('data-mvb-paddtop'),
              		...
              		row_delimiter: therow.attr('data-mvb-row_delimiter'),
					...
					video_controls: therow.attr('data-mvb-video_controls')
              	 };
    ...
    $(".mvb_submit_row_settings").live('click', function(){
        ...
        therow.attr('data-mvb-row_delimiter', $("#bshaper_tmp").find('.mvb_module_row_delimiter').val());
		...
		therow.attr('data-mvb-video_controls', $("#bshaper_tmp").find('.mvb_module_video_controls').val());
    ...
    $("a.mvb_column_settings").live('click', function(e){
        ...
        var data = {
                    ...
                    totop: thecolumn.attr('data-mvb-totop'),
              		...
              		column_highlight: thecolumn.attr('data-mvb-column_highlight'),
              		column_highlight_color: thecolumn.attr('data-mvb-column_highlight_color'),
              		column_highlight_bgimage: thecolumn.attr('data-mvb-column_highlight_bgimage'),
              		column_highlight_bgrepeat: thecolumn.attr('data-mvb-column_highlight_bgrepeat'),
              		column_highlight_bgposition: thecolumn.attr('data-mvb-column_highlight_bgposition'),
    ...
    $(".mvb_submit_column_settings").live('click', function(){
        ...
        thecolumn.attr('data-mvb-column_highlight', $("#bshaper_tmp").find('.mvb_module_column_highlight').val());
        thecolumn.attr('data-mvb-column_highlight_color', $("#bshaper_tmp").find('.mvb_module_column_highlight_color').val());
        thecolumn.attr('data-mvb-column_highlight_bgimage', $("#bshaper_tmp").find('.mvb_module_column_highlight_bgimage').val());
        thecolumn.attr('data-mvb-column_highlight_bgrepeat', $("#bshaper_tmp").find('.mvb_module_column_highlight_bgrepeat').val());
        thecolumn.attr('data-mvb-column_highlight_bgposition', $("#bshaper_tmp").find('.mvb_module_column_highlight_bgposition').val());
    ...
    $( "a.bshaper_edit_module" ).live('click', function(e) {
        ...
        $.post(ajaxurl, data, function(_html) {
            $('#bshaper_tmp').html(_html).dialog( {modal:false,height: 600,width: 700, 'title' : 'Edit module', zIndex: 100, open : mvb_dialog_open, beforeClose : mvb_dialog_close} );
            mvb_do_asm_select();
			initialize_fields();
			initialize_color_pickers();
			initTheEditor('mvb_textarea_with_editor');
        ...
        if( !field_tmp.hasClass('mvb_repeater_field') )
        {
            ...
            else
            {
                if( !field_tmp.hasClass('mvb_textarea_with_editor') ) {
					...
					if (field_tmp.attr('id') != undefined && field_tmp.attr('id') != '') {
						mvb_data[field_tmp.attr('name')] = tinyMCE.get(field_tmp.attr('id')).getContent();
					} else {
            ...
            if( !$frm.find(".bshaper_remove_section_cbx").is(':checked') )
            {
                $frm.find('.mvb_module_field').each(function(the_index){
					...
					if (repeater_field_tmp.hasClass('mvb_textarea_with_editor')) {
						if (repeater_field_tmp.attr('id') != undefined && repeater_field_tmp.attr('id') != '') {
...
function removeTinyMCE(inst) {
	...
	jQuery.each(elements, function(key, value) {
		...
		if (el.attr('id') != '' && el.attr('id') != undefined) {
			...
			id = 'mvb_twe_'+Math.random().toString(36).substring(7);
			el.attr('id', id);
...
function initTheEditor(element_class)
{
    ...
    tinyMCE.init({
        ...
        theme: "advanced",
        skin: "wp_theme",
        plugins: 'inlinepopups,spellchecker,tabfocus,paste,media,fullscreen,wordpress,wpeditimage,wpgallery,wplink,wpdialogs,wpfullscreen'+styled_button_plugin,
        theme_advanced_buttons1: "formatselect,separator,bold,italic,underline,separator,strikethrough,justifyleft,justifycenter,justifyright, justifyfull,bullist,table,separator,forecolor,backcolor,image,separator,pastetext,separator,link,unlink,separator,code"+styled_button,
        relative_urls: true,
