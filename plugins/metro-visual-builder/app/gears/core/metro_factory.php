...
//end render_meta_box()
	...
	public function render_meta_box_content($meta_value = '') {
		...
		$row_navigation = get_post_meta($post->ID, '_metro_builder_row_navigation', true);
		$scroll_navigation = get_post_meta($post->ID, '_metro_builder_scroll_navigation', true);

		if ($activate_metro != '0' &&  $activate_metro != '2' )
			$activate_metro = 1;

		$load['_bshaper_activate_metro_builder'] = $activate_metro;
		$load['_metro_builder_row_navigation'] = $row_navigation;
		$load['_metro_builder_scroll_navigation'] = $scroll_navigation;
	...
	public function editor_parse_mvb_array($arr) {
		...
		foreach ($arr as $row_id => $row) {
			$str .= '<li class="row bshaper_row" data-mvb-bgcolor="' . $row['settings']['bgcolor'] .
					...
					'" data-mvb-row_delimiter="' . ((isset($row['settings']['row_delimiter']))?$row['settings']['row_delimiter']:'') .
					...
					'" data-mvb-video_controls="' . $row['settings']['video_controls'] .
			...
			foreach ($row['columns'] as $column) {
				$str .= '<div class="' . $metro_admin_grid[$column['size']] .
						...
						'" data-mvb-column_highlight="' . (isset($column['settings']['column_highlight'])?$column['settings']['column_highlight']:'') .
						'" data-mvb-column_highlight_color="' . (isset($column['settings']['column_highlight_color'])?$column['settings']['column_highlight_color']:'') .
						'" data-mvb-column_highlight_bgimage="' . (isset($column['settings']['column_highlight_bgimage'])?$column['settings']['column_highlight_bgimage']:'') .
						'" data-mvb-column_highlight_bgrepeat="' . (isset($column['settings']['column_highlight_bgrepeat'])?$column['settings']['column_highlight_bgrepeat']:'') .
						'" data-mvb-column_highlight_bgposition="' . (isset($column['settings']['column_highlight_bgposition'])?$column['settings']['column_highlight_bgposition']:'') .
...
//end store_value()
	...
	function load_assets() {
		if (is_admin()) {
			...
			wp_register_script('metro_builder_main', $this->app_url . 'assets/js/main.js', array('jquery'), false, true);
...
// end queue_scripts()
	...
	public function parse_mvb_array($arr) {
		...
		foreach ($arr as $row) {
			...
			if (!empty($row['settings']['row_delimiter'])) {
				$row_delimiter_class = ' mvb-row-delimiter-'.$row['settings']['row_delimiter'];
			} else {
				$row_delimiter_class = '';
			}

			$row_classes = $row_padding_classes . $row_delimiter_class. ' ' . ((isset($row['settings']['cssclass_manual'])) ? $row['settings']['cssclass_manual'] : '');
			$class_darklight = '';
			...
			$columns_count = count($row['columns']) - 1;
			
			$columns_count_max = 12;
			$columns_total_count = 0;
			
			foreach ($row['columns'] as $i=>$column) {
				$columns_total_count += $column['size'];
			}
			
			if ($columns_total_count > $columns_count_max) {
				$multicolumn_row = true;
			} else {
				$multicolumn_row = false;
			}
			
			if ($multicolumn_row) {
				$str .= '<div class="subrow-wrapper-section">';
			}
			...
			foreach ($row['columns'] as $i=>$column) {
				...
				if ($columns_total_count > $columns_count_max) {
					$str .= '</div><div class="subrow-wrapper-section">';
					$columns_total_count = $column['size'];
					$columns_i = 0;
				}
				...
				$column_class = $background_check . ' ' 
								. $this->the_column_class($column['size'], $column['settings']['smallclass'], $metro_basic_grid, $metro_mobile_grid) . ' ' 
								. $column['settings']['cssclass'];
				
				if (
					isset($column['settings']['column_highlight']) && 
					$column['settings']['column_highlight'] &&
					(
						$columns_i == 0 ||
						$i == $columns_count ||
						(
							isset($row['columns'][$i+1]) &&
							(($row['columns'][$i+1]['size'] + $columns_total_count) > $columns_count_max)
						)
					)
				) {
					$highlight_class = $this->the_column_class($column['size'], $column['settings']['smallclass'], $metro_basic_grid, $metro_mobile_grid);
					
					if ($columns_i == 0) {
						$highlight_class .= ' mvb-column-highlight-left';
					}
					...
					if (
						...
						$highlight_class .= ' mvb-column-highlight-right';
					}
					
					$highlight_style = $this->build_highlight_css($column['settings']);

					$mvb_column_highlight = '<div class="'.$highlight_class.'" style="'.$highlight_style.'"></div>';
					$mvb_column_highlight_mobile = '<div class="mvb-column-highlight-mobile" style="'.$highlight_style.'"></div>';
				} else {
					$mvb_column_highlight = '';
					$mvb_column_highlight_mobile = '';
				}
				
				$str .= '<div class="' . $column_class . '">'
							. $mvb_column_highlight_mobile
							. '<div class="mvb_inner_wrapper"' . $column_css . '>'
								. do_shortcode($column['shortcodes']) 
							. '</div>'
						. '</div>'
						. $mvb_column_highlight;
			...
			if (!empty($str_video) && (empty($row['settings']['video_controls']) || $row['settings']['video_controls'] === 'on')) {
				$str .= '<div class="row-video-controls"><a class="video video-on" href="#">&nbsp;</a>'
						. '<a class="sound sound-on" href="#">&nbsp;</a></div>';
			}
...
//end parse_mvb_array();
	...
	public function build_highlight_css($settings) {
		$css = '';
		
		if (!empty($settings['column_highlight_bgimage'])) {
			$css .= 'background-image: url(' . mvb_get_image_url($settings['column_highlight_bgimage']) . ');';
		}

		if (!empty($settings['column_highlight_bgrepeat'])) {
			$css .= 'background-repeat: ' . $settings['column_highlight_bgrepeat'] . ';';
		}
		
		//column_highlight_bgposition
		if (!empty($settings['column_highlight_bgposition'])) {
			$css .= 'background-position: '.$settings['column_highlight_bgposition'].';';
		}

		if (!empty($settings['column_highlight_color'])) {
			$css .= 'background-color: #' . $settings['column_highlight_color'] . ';';
		}
