...
//end get_section_types();
	...
	public function row_settings_fields() {
		...
		$the_fields = array(
			...
			'row_delimiter' => array(
				'type' => 'select',
				'label' => __('Row Delimiter', 'mvb'),
				'default' => '0',
				'options' => mvb_row_delimiters(),
				'col_span' => 'lbl_half'
			),
...
//end row_settings_fields();
	...
	public function get_row_settings() {
		...
		$atts = array(
			...
			'row_delimiter' => $_POST['row_delimiter'],
...
//end get_row_settings();
	...
	public function columns_settings_fields() {
		...
		$the_fields = array(
			...
			'column_highlight' => array(
				'type' => 'select',
				'label' => __('Higlight', 'mvb'),
				'default' => '0',
				'options' => mvb_yes_no(),
				'col_span' => 'lbl_half'
			),
			'column_highlight_color' => array(
				'type' => 'colorpicker',
				'label' => __('Higlight Color', 'mvb'),
				'col_span' => 'lbl_forth'
			),
			'column_highlight_bgimage' => array(
				'type' => 'image',
				'label' => __('Higlight Background Image', 'mvb'),
				'col_span' => 'lbl_half'
			),
			'column_highlight_bgrepeat' => array(
				'type' => 'select',
				'label' => __('Higlight Background repeat', 'mvb'),
				'default' => '',
				'options' => mvb_get_bgrepeat(),
				'col_span' => 'lbl_half',
			),
			'column_highlight_bgposition' => array(
				'type' => 'select',
				'label' => __('Higlight Background position', 'mvb'),
				'default' => '',
				'options' => mvb_get_bgposition(),
				'col_span' => 'lbl_half',
			),
...
//end columns_settings_fields();
	...
	public function get_column_settings() {
		...
		$atts = array(
			...
			'column_highlight' => (isset($_POST['column_highlight']))?$_POST['column_highlight']:'',
			'column_highlight_color' => (isset($_POST['column_highlight_color']))?$_POST['column_highlight_color']:'',
			'column_highlight_bgimage' => (isset($_POST['column_highlight_bgimage']))?$_POST['column_highlight_bgimage']:'',
			'column_highlight_bgrepeat' => (isset($_POST['column_highlight_bgrepeat']))?$_POST['column_highlight_bgrepeat']:'',
			'column_highlight_bgposition' => (isset($_POST['column_highlight_bgposition']))?$_POST['column_highlight_bgposition']:'',
...
//end show_module()
	...
	function save_meta() {
		...
		if (!isset($_POST['post_id']) OR !is_numeric($_POST['post_id']) OR $_POST['post_id'] < 0) {
			return;
		}
...
//end column_parser
	...
	function save_meta_post() {
		...
		if (!isset($_POST['bshaper_the_post_id']) OR !is_numeric($_POST['bshaper_the_post_id']) OR $_POST['bshaper_the_post_id'] < 0) {
			return;
		}
		...
		$this->save_meta_field('_metro_builder_row_navigation', $post_id);
		$this->save_meta_field('_metro_builder_scroll_navigation', $post_id);
	}
	...
	function html_parse($_html_brute) {
		...
		foreach (pq('.bshaper_row') as $row) {
			...
			$arr[$row_id]['settings'] = array(
				...
				'row_delimiter' => pq($row)->attr('data-mvb-row_delimiter'),
				...
				'video_controls' => pq($row)->attr('data-mvb-video_controls'),
			);
			...
			foreach (pq($row)->find('.columns') as $column) {
				...
				$settings = array(
					...
					'column_highlight' => pq($column)->attr('data-mvb-column_highlight'),
					'column_highlight_color' => pq($column)->attr('data-mvb-column_highlight_color'),
					'column_highlight_bgimage' => pq($column)->attr('data-mvb-column_highlight_bgimage'),
					'column_highlight_bgrepeat' => pq($column)->attr('data-mvb-column_highlight_bgrepeat'),
					'column_highlight_bgposition' => pq($column)->attr('data-mvb-column_highlight_bgposition'),
...
//					'video_controls' => pq($column)->attr('data-mvb-video_controls'),
				);
