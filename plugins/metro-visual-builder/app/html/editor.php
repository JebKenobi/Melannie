...
<div class="bshaper_metro_settings">
	...
	<label>
        <span><?php _e('Activate Row Navigation?', 'mvb') ?></span>
        <select name="_metro_builder_row_navigation" class="input_select">
            <?php $yes_no = mvb_yes_no() ?>
            <?php foreach( $yes_no as $value => $lbl ): ?>
                  <option value="<?php echo $value ?>" <?php if( $value == $_metro_builder_row_navigation ): ?> selected="selected" <?php endif; ?>> <?php echo $lbl ?> </option>
            <?php endforeach; ?>
        </select>
    </label>
	
	<label>
        <span><?php _e('Activate Scroll Navigation?', 'mvb') ?></span>
        <select name="_metro_builder_scroll_navigation" class="input_select">
            <?php $yes_no = mvb_yes_no() ?>
            <?php foreach( $yes_no as $value => $lbl ): ?>
                  <option value="<?php echo $value ?>" <?php if( $value == $_metro_builder_scroll_navigation ): ?> selected="selected" <?php endif; ?>> <?php echo $lbl ?> </option>
            <?php endforeach; ?>
        </select>
    </label>
    <div class="clear"><!-- ~ --></div>
